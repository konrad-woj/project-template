...
sh run_on_each.sh "uv run task precommit"
sh run_on_each.sh "uv lock"
...
{repo_root}/
  packages/
    {package-name}
      ...
      evaluation/
        __init__.py
        {evaluation_name}.py  # evaluation scripts, run locally with --dev dependency group, not meant to be imported in packages, e.g., evaluation of model performance, evaluation of retrieval quality, etc.
      scripts/
        __init__.py
        {script_name}.py  # other dev scripts for local testing
  data-utils/
    src/data_utils/
      __init__.py
      {util_name}.py
...
- Inside the package dir you must always:
  ...
  - Put all generated .md files in `./docs` dir
  ...
  - Evaluation scripts can be found in `./evaluation` dir
  - Executable scripts can be found in `./scripts` dir
  - When changing the API, always update api docs with `uv run task save-api` 
  ...
  - For the execution timing use `timer` or `async_timer` modules from `../logger` package (ideally as decorators)
  - Generic, cross-package utlities are located in `../data-utils` dir - check it before implementing new utilities locally
  - Generic, cross-package pydantic models are located in `../data-models` dir - check it before implementing new models locally
  - For API contracts (input/output messages) use our BaseSchema class from `../data-models` instead of Pydantic BaseModel
